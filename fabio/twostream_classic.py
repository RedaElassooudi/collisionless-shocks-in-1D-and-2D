"""
    ...
    For the course Introduction to Plasma Dynamics by Fabio Bacchini and Rony Keppens
    KU Leuven
...
L = 3 * 2 * np.pi / (np.sqrt(3.0 / 2) / 2.0)  # Domain size
DT = 0.02  # Time step
...
WP = 1.0  # Plasma frequency w_p = sqrt((n * q^2) / (m * eps))
...
# Q / QM = WP^2 / (QM * N / L) / QM = (WP^2 * L) / QM = Q * N / (WP^2 * L)
# Q / QM = wp^2 / (N/L) =
# Q / QM = n * e^2 / (eps0 * me * qm^2 * N/L) = e^2 / (eps0 * me * qm^2)? (yes if n = N/L)
# = 1 for electrons
# = 1836 for ions
...
for it in range(NT + 1):
    ...
    Ekin = 0.5 * Q / QM * (vp**2).sum()
    histEnergy.append(Epot + Ekin)
    histPotE.append(Epot)
    histKinE.append(Ekin)
